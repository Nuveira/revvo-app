...
$pageTitle = 'Tambah Motor | REVVO';
require_once '../../config/koneksi.php';
require_once '../../includes/auth.php';
require_once '../../includes/customer_role.php';
require_once '../../includes/customer_motor_helpers.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM motors WHERE customer_id = ? AND plate_number = ?");
        $stmt->bind_param("is", $customer_id, $plateNumber);
        $stmt->execute();
        $existingPlate = (int) $stmt->get_result()->fetch_assoc()['total'];
        $stmt->close();

        if ($existingPlate > 0) {
            $errors['plate_number'] = 'Nomor plat ini sudah terdaftar.';
        }
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO motors (customer_id, brand, model, plate_number, production_year, color, image_path)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("isssiss", $customer_id, $brand, $model, $plateNumber, $productionYear, $color, $imagePath);

        if ($stmt->execute()) {
            $stmt->close();
            $_SESSION['motor_success'] = 'Motor berhasil ditambahkan.';
            header('Location: ' . url('pages/customer/motor.php'));
            exit;
        }

        $errors['form'] = 'Motor gagal ditambahkan. Coba lagi.';
        $stmt->close();
    }
}
?>
        ...
        <main class="flex-1 min-w-0 bg-gray-100 overflow-y-auto overflow-x-hidden">
            <div class="bg-gradient-to-r from-black via-black via-20% to-[#8E1616] flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
                <div class="min-w-0">
                    ...
                    <h1 class="mt-2 text-2xl sm:text-4xl text-white font-semibold break-words">Tambah Motor Baru</h1>
                </div>
            ...
            <div class="p-4 w-full mx-auto">
                ...
                <div class="rounded-lg border border-[#eadede] bg-white p-5 shadow-sm">
                    <div class="mb-5">
                        ...
                        <h2 class="mt-2 text-xl font-semibold text-gray-900">Tambah motor</h2>
                    </div>

                    <form method="POST" action="tambah_motor.php" enctype="multipart/form-data" class="space-y-4">
                        <div>
                        ...
                        <div>
                            <label for="image" class="text-sm font-medium text-gray-700">Foto Motor <span class="text-gray-400 font-normal">(opsional, maks. 2MB)</span></label>
